...
if( !function_exists('delicious_enqueue_scripts') ) {
	...
	function delicious_enqueue_scripts() {
		...
		wp_enqueue_style( 'dt-delicious-font', "".$protocol."://fonts.googleapis.com/css?family=Open+Sans:300italic,400,300,600,700,800");
		...
		wp_localize_script( 'dt-custom-nav', 'dt_handler', array( 'curlink' => $current_link) );
		...
		wp_localize_script( 'dt-custom-navscroll', "dt_styles", array( 'header_bg' => $header_color, 'header_scroll_opacity' => $scroll_op, 'default_color' => $default_header_color, 'logo_width' => $logo_details[1], 'logo_height' => $logo_details[2], 'init_pt' => $init_pt, 'init_pb' => $init_pb, 'scroll_pt' => $scroll_pt, 'scroll_pb' => $scroll_pb, 'scrolling_effect' => $scrolling_effect ) );			
